...
describe("Schema Version System", () => {
  ...
  test("versions.json has valid structure", () => {
    ...
    expect(
      originalVersionInfo.compatibility[originalVersionInfo.minimum],
    ).toBeDefined();
...
describe("Chopd Version Integration", () => {
  ...
  beforeEach(() => {
    ...
    fs.writeFileSync(
      TEMP_VERSIONS_PATH,
      JSON.stringify(versionsContent, null, 2),
    );
  });
  ...
  test("chopd version bump updates package.json and versions.json", () => {
    ...
    const packageInfoBefore = JSON.parse(
      fs.readFileSync(TEMP_PACKAGE_PATH, "utf8"),
    );
    const versionInfoBefore = JSON.parse(
      fs.readFileSync(TEMP_VERSIONS_PATH, "utf8"),
    );
    ...
    fs.writeFileSync(
      TEMP_PACKAGE_PATH,
      JSON.stringify(packageInfoBefore, null, 2),
    );

    // 2. Update versions.json compatibility
    Object.keys(versionInfoBefore.compatibility).forEach((schemaVersion) => {
      if (
        versionInfoBefore.compatibility[schemaVersion] === chopdVersionBefore
      ) {
        versionInfoBefore.compatibility[schemaVersion] =
          `${expectedPatchVersion}+`;
      }
    });
    fs.writeFileSync(
      TEMP_VERSIONS_PATH,
      JSON.stringify(versionInfoBefore, null, 2),
    );

    // Verify the changes
    const packageInfoAfter = JSON.parse(
      fs.readFileSync(TEMP_PACKAGE_PATH, "utf8"),
    );
    expect(packageInfoAfter.version).toBe(expectedPatchVersion);

    const versionInfoAfter = JSON.parse(
      fs.readFileSync(TEMP_VERSIONS_PATH, "utf8"),
    );
    Object.entries(versionInfoAfter.compatibility).forEach(
  ...
  test("chopd version bump with specific schema versions updates compatibility mapping", () => {
    ...
    versionInfoBefore.compatibility[testSchemaVersion] =
      `${expectedPatchVersion}+`;
    fs.writeFileSync(
      TEMP_VERSIONS_PATH,
      JSON.stringify(versionInfoBefore, null, 2),
    ...
    expect(versionInfoAfter.compatibility[testSchemaVersion]).toBe(
      `${expectedPatchVersion}+`,
    );
  });
});
...
describe("Schema Version Actual Modifications", () => {
  ...
  beforeEach(() => {
    ...
    const schemaContent = fs.readFileSync(
      path.join(__dirname, "schemas/schema_test.json"),
      "utf8",
    );
...
`;
  ...
  function createTempBumpScript() {
    const bumpScriptPath = path.join(
      __dirname,
      "scripts/bump-schema-version.js",
    );
    ...
    const modifiedScriptContent = bumpScriptContent
      .replace(
        /const versionsPath =.*/g,
        `const versionsPath = '${TEMP_VERSIONS_PATH}';`,
      )
      .replace(
        /const schemasDir =.*/g,
        `const schemasDir = '${TEMP_SCHEMAS_DIR}';`,
      )
      .replace(
        /const readmePath =.*/g,
        `const readmePath = '${TEMP_README_PATH}';`,
      );
  ...
  test("patch bump creates new schema file and updates versions.json", () => {
    ...
    // Run the bump script
    const result = spawnSync("node", [tempScriptPath, "patch"], {
      encoding: "utf8",
    });
    ...
    const newSchemaPath = path.join(
      TEMP_SCHEMAS_DIR,
      `schema_${expectedPatchVersion}.json`,
    );
    ...
    expect(versionInfoAfter.history.length).toBe(
      versionInfoBefore.history.length + 1,
    );
  ...
  test("minor bump creates new schema file and updates versions.json", () => {
    ...
    // Run the bump script
    const result = spawnSync("node", [tempScriptPath, "minor"], {
      encoding: "utf8",
    });
    ...
    const newSchemaPath = path.join(
      TEMP_SCHEMAS_DIR,
      `schema_${expectedMinorVersion}.json`,
    );
    expect(fs.existsSync(newSchemaPath)).toBe(true);
  });
  ...
  test("custom chopd version is added to compatibility mapping", () => {
    ...
    const result = spawnSync(
      "node",
      [tempScriptPath, "patch", "--chopd-version", customVersion],
      { encoding: "utf8" },
    );
    ...
    expect(versionInfoAfter.compatibility[expectedPatchVersion]).toBe(
      customVersion,
    );
  });
});
