...
if (schemaVersionsIndex !== -1 && args.length > schemaVersionsIndex + 1) {
  schemaVersions = args[schemaVersionsIndex + 1].split(',').map(v => v.trim());
  
  // Validate schema versions
  for (const version of schemaVersions) {
    if (!semver.valid(version)) {
      console.error(`Error: Invalid schema version "${version}". Please provide valid semver versions.`);
      process.exit(1);
    }
  }
}
...
try {
  ...
  if (isTestMode) {
    ...
    console.log(`Created temporary package.json for testing: ${tempPackagePath}`);
  }
} catch (err) {
...
try {
  ...
  if (isTestMode) {
    ...
    console.log(`Created temporary versions.json for testing: ${tempVersionsPath}`);
  }
} catch (err) {
...
console.log(`Bumping chopd version from ${currentVersion} to ${newVersion} (${bumpType})`);
...
try {
  ...
  if (!isTestMode) {
    fs.writeFileSync(actualPackagePath, JSON.stringify(packageInfo, null, 2) + '\n');
  }
  console.log(`${isTestMode ? '[TEST] Would update' : 'Updated'} package.json with new version: ${newVersion}`);
} catch (err) {
...
try {
  ...
  if (schemaVersions.length === 0) {
    Object.entries(versionInfo.compatibility).forEach(([schemaVersion, chopdVersion]) => {
      if (chopdVersion === currentVersion || chopdVersion.endsWith(`${currentVersion}+`)) {
    ...
    console.log(`No specific schema versions provided. Using: ${schemaVersions.join(', ')}`);
  }
  ...
  for (const schemaVersion of schemaVersions) {
    ...
    if (!versionInfo.compatibility[schemaVersion]) {
      ...
      if (schemaVersion !== versionInfo.current && 
          !versionInfo.history.some(h => h.version === schemaVersion)) {
        console.warn(`Warning: Schema version ${schemaVersion} is not recognized in version history. Adding anyway.`);
      }
    }
  ...
  console.log(`${isTestMode ? '[TEST] Would update' : 'Updated'} versions.json compatibility mapping for schema versions: ${schemaVersions.join(', ')}`);
} catch (err) {
...
try {
  ...
  if (match) {
    ...
    for (const schemaVersion of schemaVersions) {
      const schemaVersionRow = new RegExp(`\\| ${schemaVersion}\\s*\\|\\s*[^\\|]*\\|`, 'm');
    ...
    console.log(`${isTestMode ? '[TEST] Would update' : 'Updated'} README.md with new chopd version: ${newVersion}`);
  } else {
