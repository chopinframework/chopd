...
for (const testCase of testCases) {
  ...
  if (result.status === 0) {
    ...
    console.log(`Command executed: node ${bumpScriptPath} ${testCase.args.join(' ')}`);
    
    // Log a brief summary of what was simulated
    const outputLines = result.stdout.split('\n');
    const versionLine = outputLines.find(line => line.includes('Bumping chopd version from'));
    if (versionLine) {
...
if (allPassed) {
  ...
  console.error('❌ Some tests failed. Please review the errors above and fix the issues.');
  process.exit(1);
} 