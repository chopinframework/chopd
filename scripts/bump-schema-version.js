...
if (chopdVersionIndex !== -1 && args.length > chopdVersionIndex + 1) {
  ...
  if (!semver.valid(chopdVersion)) {
    console.error(
      `Error: Invalid chopd version "${chopdVersion}". Please provide a valid semver version.`,
    );
    process.exit(1);
  }
} else {
  ...
  try {
    ...
    console.error(
      "Error: Could not determine chopd version. Please provide it with --chopd-version",
    );
    process.exit(1);
  }
}
...
try {
  ...
  if (isTestMode) {
    ...
    console.log(
      `Created temporary versions file for testing: ${tempVersionsPath}`,
    );
  }
} catch (err) {
...
console.log(
  `Bumping schema version from ${currentVersion} to ${newVersion} (${bumpType})`,
);

// Define paths for current and new schema files
const currentSchemaPath = path.join(
  schemasDir,
  `schema_${currentVersion}.json`,
);
...
try {
  ...
  console.log(
    `${isTestMode ? "[TEST] Would create" : "Created"} new schema file: ${newSchemaPath}`,
  );
} catch (err) {
...
try {
  ...
  console.log(
    `${isTestMode ? "[TEST] Would update" : "Updated"} versions.json with new version: ${newVersion}`,
  );
} catch (err) {
...
try {
  ...
  if (match) {
    ...
    if (!tableRows.includes(`| ${newVersion}`)) {
      ...
      console.log(
        `${isTestMode ? "[TEST] Would update" : "Updated"} README.md compatibility table with new version: ${newVersion}`,
      );
    } else {
      console.log(
        `README.md already contains entry for version ${newVersion}, no update needed`,
      );
    }
  } else {
    console.log(
